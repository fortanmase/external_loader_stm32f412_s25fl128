...
int32_t S25FL128S_WritePages(QSPI_HandleTypeDef *Ctx, S25FL128S_Interface_t Mode,
                             uint8_t *pData, uint32_t WriteAddr, uint32_t Size)
{
    QSPI_CommandTypeDef s_command;
    uint32_t current_size, current_addr, end_addr;

    /* Calculate initial page aligned size */
    current_size = 256 - (WriteAddr % 256);
    ...
    do
    {
        ...
        if (S25FL128S_AutoPollingMemReady(Ctx, Mode) != S25FL128S_OK)
        {
            return S25FL128S_ERROR;
        }
...
int32_t S25FL128S_AutoPollingMemReady(QSPI_HandleTypeDef *Ctx, S25FL128S_Interface_t Mode)
{
    ...
    if (HAL_QSPI_AutoPolling(Ctx, &s_command, &s_config, S25FL128S_BULK_ERASE_MAX_TIME) != HAL_OK)
    {
        return S25FL128S_ERROR;
    }
